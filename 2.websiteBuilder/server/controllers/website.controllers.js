...
--------------------------------------------------
USE THESE EXACT IMAGE URLs IN ORDER:
{IMAGE_URLS}
- Use these URLs directly in <img src="...">
...
--------------------------------------------------
FEATURE-SPECIFIC FUNCTIONALITY (MANDATORY)
--------------------------------------------------
READ THE USER REQUIREMENT AND IMPLEMENT ACCORDINGLY:

IF user wants e-commerce / shop / store:
  ✔ Product grid with real prices
  ✔ Working Add to Cart button (updates cart count)
  ✔ Cart sidebar/page showing items + total
  ✔ Remove from cart functionality
  ✔ Checkout form with validation

IF user wants food delivery / restaurant:
  ✔ Menu with categories (filter by category)
  ✔ Add to cart with quantity control
  ✔ Cart with total price calculation
  ✔ Order placement form

IF user wants calculator:
  ✔ Fully working calculator with all operations
  ✔ Clear, backspace, decimal support

IF user wants games / app store:
  ✔ Filter by free/paid
  ✔ Search functionality
  ✔ Add to wishlist or cart

IF user wants dashboard:
  ✔ Real charts using Canvas API
  ✔ Interactive data tables

ALL BUTTONS MUST BE FUNCTIONAL — NO DEAD UI
--------------------------------------------------
...
export const generateWebsite = async (req, res) => {
    try {
        ...
        const imageList = images.map((url, i) => `Image ${i + 1}: ${url}`).join('\n')
        const finalPrompt = masterPrompt
        .replace("{USER_PROMPT}", prompt)
        .replace("{IMAGE_URLS}", imageList)
        ...
        images[0] || 'https://picsum.photos/400/300'
        )
        }
        //---
